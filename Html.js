class Html {
    ...
    setStyle(property, value) {
        ...
        const styles = this.attrs.style.split(';').filter(Boolean).reduce((acc, style) => {
            const [key, val] = style.split(':').map(s => s.trim());
            if (key) acc[key] = val;
            return acc;
        }, {});
        styles[property] = value;
        this.attrs.style = Object.entries(styles).map(([k, v]) => `${k}: ${v}`).join('; ');
        return this;
    }
    ...
    toHtml() {
        ...
        const attrs = Object.entries(this.attrs).map(([k, v]) => v === true ? k : v === false || v == null ? '' : `${k}="${HtmlElement.escapeAttr(v)}"`).filter(Boolean).join(' ');
