...
// ===== FlowUploader Integration =====
// This section integrates Flow.js with the HtmlElement DOM builder in a reusable way.

class FlowUploader {
  constructor({ target, browseId, dropId, handlers = {} }) {
    if (typeof Flow === 'undefined') {
      throw new Error('Flow.js library not loaded. Please include flow.js before using FlowUploader.');
    }
    ...
    const browseElement = document.getElementById(browseId);
    const dropElement = document.getElementById(dropId);

    if (browseElement) this.flow.assignBrowse(browseElement);
    if (dropElement) this.flow.assignDrop(dropElement);
...
// ===== Example Usage =====
// const uploader = new FlowUploader({
//   target: '/upload',
//   browseId: 'browseBtn',
//   dropId: 'dropZone',
//   handlers: {
...
//       uploader.upload();
//     },
//     fileSuccess: (file, message) => console.log('Uploaded:', file.name, message),
//     fileError: (file, message) => console.error('Error:', file.name, message)
//   }
// });
