 ...
 * - **NEW** Reusable, vanilla JS integration with JSZip + FileSaver for backup/restore & import/export
 *
 * Works as both an ES module (export) and a global (window.UniversalStorage) for vanilla usage.
 * If used as a global, include the following in HTML *before* using the zip methods:
...
(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    const api = factory();
    root.UniversalStorage = api.UniversalStorage;
    root.storage = api.storage;
  }
})(typeof self !== 'undefined' ? self : this, function () {
  // ---------------- JSZip + FileSaver helpers (vanilla friendly) ----------------
  const ZipIO = {
    ensureDeps() {
      ...
      if (!hasJSZip || !hasSaveAs) {
        const err = new Error(
          'Zip features require JSZip and FileSaver. Include JSZip and FileSaver via <script> tags or import them before using zip methods.'
        );
        err.missing = { JSZip: !hasJSZip, FileSaver: !hasSaveAs };
        throw err;
      }
    },

    async exportJSONToZip({ filename = 'backup.zip', files = {} }) {
      this.ensureDeps();
      const zip = new JSZip();
      // files: { 'data.json': jsonString, 'meta.json': jsonString, ... }
    ...
    async readZipJSON(file) {
      this.ensureDeps();
      const zip = await JSZip.loadAsync(file);
      // Prefer data.json; otherwise read all .json files
      const candidates = Object.keys(zip.files).filter(k => k.toLowerCase().endsWith('.json'));
      if (!candidates.length) throw new Error('No JSON files found in ZIP.');
      const out = {};
      for (const name of candidates) {
        out[name] = JSON.parse(await zip.file(name).async('string'));
      }
      return out;
    },
  };
  ...
  class UniversalStorage {
    ...
    where(field, operator, value) {
      this.filters.push({ kind: 'and', field, operator: String(operator).toUpperCase(), value });
      return this;
    }
    orWhere(field, operator, value) {
      this.filters.push({ kind: 'or', field, operator: String(operator).toUpperCase(), value });
      return this;
    }
    whereIn(field, arr) { this.filters.push({ kind: 'and', field, operator: 'IN', value: arr }); return this; }
    whereBetween(field, min, max) { this.filters.push({ kind: 'and', field, operator: 'BETWEEN', value: [min, max] }); return this; }
    whereNull(field) { this.filters.push({ kind: 'and', field, operator: 'IS NULL', value: null }); return this; }
    whereNotNull(field) { this.filters.push({ kind: 'and', field, operator: 'IS NOT NULL', value: null }); return this; }
    orderBy(field, direction = 'asc') { this.sortField = field; this.sortDirection = (direction || 'asc').toLowerCase() === 'desc' ? 'desc' : 'asc'; return this; }
    limit(count) { this.limitCount = Number(count); return this; }
    paginate(page, perPage) { this.pageNumber = Math.max(1, Number(page) || 1); this.perPageCount = Math.max(1, Number(perPage) || 10); return this; }
    ...
    async first() { return (await this.limit(1).get())[0] ?? null; }
    async findById(id) { const all = await this._fetchAll(); return all.find(it => it[this.idKey] === id) || null; }
    async count() { const data = await this.get(); return data.length; }
    async exists() { return (await this.limit(1).get()).length > 0; }
    async pluck(field) { const data = await this.get(); return data.map(r => r?.[field]); }
    async distinct(field) { const data = await this.get(); return [...new Set(data.map(r => r?.[field]))]; }

    // ---------- Aggregates ----------
    async sum(field) { const data = await this.get(); return data.reduce((acc, r) => acc + (Number(r?.[field]) || 0), 0); }
    async avg(field) { const data = await this.get(); return data.length ? (await this.sum(field)) / data.length : 0; }
    async min(field) { const data = await this.get(); if (!data.length) return null; return data.reduce((m, r) => (m == null || r?.[field] < m ? r?.[field] : m), null); }
    async max(field) { const data = await this.get(); if (!data.length) return null; return data.reduce((m, r) => (m == null || r?.[field] > m ? r?.[field] : m), null); }
    ...
    async join(
      ...
      localField,
      foreignField,
      { type = 'inner', select = (a, b) => ({ ...a, ...b }), aliasA, aliasB } = {}
    ) {
      ...
      // After a join, return a lightweight wrapper
      return {
        toArray: () => results,
     ...
     */
    async sql(command) {
      const sql = String(command).trim().replace(/\s+/g, ' ');
    ...
    // ---------- Import / Export / Backup (NEW) ----------
    /**
     * Dump current store rows to plain array.
     */
    async dumpArray() {
      const all = await this._fetchAll();
    ...
    /**
     * Export all rows as a JSON string. Useful for manual downloads or debugging.
     */
    async exportJSON({ pretty = true } = {}) {
      const rows = await this.dumpArray();
      return pretty ? JSON.stringify(rows, null, 2) : JSON.stringify(rows);
    }

    /**
     * Trigger a browser download of data.json (no zip). Uses Blob + URL; no FileSaver needed here.
     */
    async downloadJSON(filename = `${this.storeName}.json`, { pretty = true } = {}) {
      const json = await this.exportJSON({ pretty });
    ...
    /**
     * Create a ZIP and download it using FileSaver.saveAs. Requires JSZip + FileSaver globals.
     */
    async downloadZIP(filename = `${this.storeName}.zip`, { pretty = true, meta = {} } = {}) {
      const rows = await this.dumpArray();
      const payload = {
      ...
      const files = {
        'data.json': pretty ? JSON.stringify(rows, null, 2) : JSON.stringify(rows),
        'meta.json': JSON.stringify({ ...payload, ...meta }, null, 2),
      };
      return ZipIO.exportJSONToZip({ filename, files });
    }

    /**
     * Import from an array of objects. Strategy options:
     *  - replace: clear store then insert all
     *  - merge: upsert by idKey (default)
     *  - skip: only insert if id not present
     */
    async importArray(rows, { strategy = 'merge' } = {}) {
      if (!Array.isArray(rows)) throw new Error('importArray expects an array of objects');
      if (strategy === 'replace') {
        await this.clear();
      ...
      const all = await this._fetchAll();
      const index = new Map(all.map(r => [r?.[this.idKey], r]));
      for (const r of rows) {
        ...
        if (id == null || !index.has(id)) {
          ...
          await this.update(id, { ...index.get(id), ...r });
          updated++;
        } else if (strategy === 'skip') {
          skipped++;
        } else {
          // default to merge behavior if unknown
          await this.update(id, { ...index.get(id), ...r });
          updated++;
        }
      }
     ...
     */
    async importJSON(jsonText, opts) {
      const data = JSON.parse(jsonText);
      return this.importArray(data, opts);
    }

    /**
     * Read a ZIP File/Blob, parse JSON files inside, and import the first array found in data.json (or any *.json with an array root).
     */
    async importZIP(file, opts) {
      const files = await ZipIO.readZipJSON(file);
      // Prefer data.json
      if (files['data.json'] && Array.isArray(files['data.json'])) {
        return this.importArray(files['data.json'], opts);
      }
      // Otherwise pick the first array-valued JSON file
      for (const v of Object.values(files)) {
        if (Array.isArray(v)) return this.importArray(v, opts);
      }
      throw new Error('No array root found in any JSON file within the ZIP.');
    }
    ...
    _applyWhere(rows, filters) {
      ...
      return rows.filter(row => {
        ...
        for (const f of filters) {
          ...
          if (f.kind === 'and') andOK = andOK && passed; else if (f.kind === 'or') orOK = orOK || passed;
        }
        return (andOK || orOK);
      });
    }
    ...
    _parseCondition(cond) {
      ...
      m = cond.match(/^([A-Za-z0-9_.$]+)\s*(=|!=|>=|<=|>|<|LIKE)\s*(.+)$/i);
      if (m) {
        const field = m[1];
    ...
    _nextLocalId(items) { const max = items.reduce((m, r) => Math.max(m, Number(r?.[this.idKey]) || 0), 0); return max + 1; }
  }

  /* Convenience factory */
  async function storage(opts) { const s = new UniversalStorage(opts); await s.init(opts?.schemaUpgrade); return s; }

  // If used as ES module
  try { if (typeof exports === 'object') { exports.UniversalStorage = UniversalStorage; exports.storage = storage; } } catch (_) {}

  return { UniversalStorage, storage };
});
